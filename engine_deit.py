...
def train_one_epoch(model: torch.nn.Module, target_model: torch.nn.Module, criterion: torch.nn.Module,
                    ...
                    memax_weight=1.0, ent_weight=0.0
                    ):
    ...
    for samples, _ in metric_logger.log_every(data_loader, print_freq, header):             
        ...
        with torch.cuda.amp.autocast(enabled=amp):
            ...
            # outputs = model.module.forward_features(samples, patch_drop=0.0)
            # print("samples:",len(smaples))
            # print("samples[0]:",len(smaples))
            
            ## Send Rand and Focal views to anchor encoder
            outputs_anchor = model.forward(samples_list[1:], msn_pretraining)
            with torch.no_grad():
                outputs_target = target_model.forward(samples_list[0], False)
                outputs_target = outputs_target.detach()
            # print("outputs_anchor.shape:",outputs_anchor.shape)
            # print("outputs_target.shape:",outputs_target.shape)         
            # exit(0)   
