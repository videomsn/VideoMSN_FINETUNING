...
def train_one_epoch(model: torch.nn.Module, run, wandb, criterion: torch.nn.Module,
    ...
    for step, (samples, targets) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        with torch.cuda.amp.autocast(enabled=amp):
            ...
            outputs = model.forward(samples, msn_pretraining)
        ...
        if amp:
            loss_scaler(loss, optimizer, clip_grad=max_norm,
                        parameters=model.parameters(), create_graph=is_second_order, update_grad=(accum_iter % grad_accum_steps == 0))
        else:
            loss.backward(create_graph=is_second_order)

        if accum_iter % grad_accum_steps == 0:
            if not amp and max_norm is not None and max_norm != 0.0:
