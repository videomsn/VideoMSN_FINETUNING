...
class VideoDataSet(data.Dataset):
    ...
    def __init__(self, root_path, list_file, num_groups=64, frames_per_group=1, sample_offset=0, num_clips=1,
                 ...
                 duration=8, super_img_rows=1, rand_size=224, focal_size=96):
        """
    ...
    def __getitem__(self, index):
        ...
        # print(images[0])
        # images[0].save("images0.png")
        # exit(0)
        
        # images = self.transform(images)
        img_views = []
        # pad_frames = 1
        # print("before self.duration:",self.duration)
        ...
        # print("after self.duration:",self.duration)
        # print("self.super_img_rows:",self.super_img_rows)
        # print("self.frame_padding:",self.frame_padding)
        # exit(0)
        # img_views += [torch.cat(self.rand_transform(images), torch.zeros(pad_frames*3, 224, 224)) for i in range(self.rand_views) if pad_frames > 0 else self.rand_transform(images)]
        
        # img_views += [
        #     torch.cat([self.rand_transform(images), torch.zeros((self.frame_padding * 3, self.rand_size, self.rand_size))]) 
        #     if self.frame_padding > 0 else self.rand_transform(images)
        #     for _ in range(self.rand_views)
        # ]
        #
        # img_views += [
        #     torch.cat([self.focal_transform(images), torch.zeros((self.frame_padding * 3, self.focal_size, self.focal_size))]) 
        #     if self.frame_padding > 0 else self.focal_transform(images)
        #     for _ in range(self.focal_views)
        # ]
        
        # print("len(img_views):",len(img_views))
        # print("img_views[0].shape:",img_views[0].shape)
        # print("img_views[1].shape:",img_views[1].shape)
        # print("img_views[2].shape:",img_views[2].shape)
        # print("img_views[3].shape:",img_views[3].shape)
        # print("img_views[7].shape:",img_views[7].shape)
        ...
        #len(img_views) = 8
        ## [2 24 224 224] [2 24 224 224] [2 24 96 96] [2 24 96 96] [2 24 96 96] [2 24 96 96] [2 24 96 96] [2 24 96 96]
        
        img_views_with_pf=[]
        for x in img_views:
            # print("Inside dataloader x.shape:",x.shape)
            x_padding=torch.zeros((3,) + x.size()[1:])
            x = torch.cat((x, x_padding), dim=0)
            img_views_with_pf+=[x]
            
        img_views=img_views_with_pf
        
        #len(img_views) = 8
        ## [2 27 224 224] [2 27 224 224] [2 27 96 96] [2 27 96 96] [2 27 96 96] [2 27 96 96] [2 27 96 96] [2 27 96 96]
        
        ## Compute pad frames values from duration and superimgrows 
        
        
        # print(len(img_views))
        # print(img_views[0].shape)
        # print(img_views[1].shape)
        
        # x= img_views[0].reshape(9,3,224,224) #(8, 3, 224, 224)
        # images=x
        # images = rearrange(images, '(th tw) c h w -> c (th h) (tw w)', th=3,c=3)
        # print("x.shape affert rearrange:",images.shape)
        #
        # images = images.numpy()
        #
        # super=images.transpose(1,2,0)
        # super=Image.fromarray((super*255).astype(np.uint8))
        # super.save('super_new.png')
        
        # exit(0)
        #
        # fsize=96
        # x= img_views[1].reshape(8,3,fsize, fsize) #(8, 3, 96, 96)
        # images = np.concatenate((x,np.expand_dims(np.zeros(x[0].shape,dtype=np.uint8), axis=0)),axis=0)
        # images = rearrange(images, '(th tw) c h w -> c (th h) (tw w)', th=3,c=3)
        # super=images.transpose(1,2,0)
        # super=Image.fromarray((super*255).astype(np.uint8))
        # super.save('super_focal_01.png')
        #
        # x= img_views[2].reshape(9,3,fsize, fsize) #(8, 3, 96, 96)
        # images = x
        # # images = np.concatenate((x,np.expand_dims(np.zeros(x[0].shape,dtype=np.uint8), axis=0)),axis=0)
        # images = rearrange(images, '(th tw) c h w -> c (th h) (tw w)', th=3,c=3)
        # images = images.numpy()
        # super=images.transpose(1,2,0)
        # super=Image.fromarray((super*255).astype(np.uint8))
        # super.save('super_focal_02_new.png')
        # #
        # x= img_views[3].reshape(9,3,fsize, fsize) #(8, 3, 96, 96)
        # images = x
        # # images = np.concatenate((x,np.expand_dims(np.zeros(x[0].shape,dtype=np.uint8), axis=0)),axis=0)
        # images = rearrange(images, '(th tw) c h w -> c (th h) (tw w)', th=3,c=3)
        # images = images.numpy()
        # super=images.transpose(1,2,0)
        # super=Image.fromarray((super*255).astype(np.uint8))
        # super.save('super_focal_03_new.png')
        
        # print("len(img_views):",len(img_views))
        # print("type(img_views):",type(img_views))
        # print("img_views[0].shape:",img_views[0].shape)
        # print("type(img_views[0]):",type(img_views[0]))
        # print("img_views[1].shape:",img_views[1].shape)
        # print("img_views[2].shape:",img_views[2].shape)
        # print("img_views[3].shape:",img_views[3].shape)
        # print("img_views[7].shape:",img_views[7].shape)
...
class VideoDataSetOnline(VideoDataSet):
    ...
    def __init__(self, root_path, list_file, num_groups=8, frames_per_group=1, sample_offset=0,
        ...
        super().__init__(root_path, list_file, num_groups, frames_per_group, sample_offset,
                         ...
                         duration, super_img_rows, rand_size, focal_size)
